...
- Following entities were introduced: `Bookshelf`, `Book`, `Statistics`. Although existence of different POJO entities more than `Bookshelf` seems to be not necessary for the task resolution, `Book`'s are still populated to follow the [requirements](#requirements).
